...
user_esp_platform_maintainer(void *pvParameters)
{
	...
	/*读取存储在flash中的相关信息（包括读取token等到结构体esp_param）*/
    ...
    if( REBOOT_MAGIC != user_boot_flag && 0 == user_get_key_status()){
        ...
        if(0){/*for test*/
            ...
            sprintf(sta_config->ssid, "PLUG");
            sprintf(sta_config->password, "Hisense");
        ...
        //如果不能搜索到flash里面存储的wifi信号，则启动smartconfig任务
        if(0 == ret) {
        ...
        //否则查询模块是否已获取ip，若未获取ip则设置模块为station+ap模式，并且设置自动连接。
        else {
       ...
       if(wifi_station_get_connect_status()==STATION_GOT_IP) 
 		{
 			...
 			ESP_DBG("Connected to %s\r\n",config.ssid);
 			ESP_DBG("\r\nGet ip:%d\r\n",info.ip.addr);
 			user_esp_platform_set_token("3f0e7693a4fe2a5abdbd562d0784c30e286c7e36");
 		}
 		else
 		{
 			wifi_station_set_auto_connect(TRUE);

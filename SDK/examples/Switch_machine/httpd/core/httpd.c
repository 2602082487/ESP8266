...
{
	//char buff[]="<html><body><form><h4>time1:<input type=\"text\" name=\"time1\" value=\"\"/><br/>time2:<input type=\"text\" name=\"time2\" value=\"\"/><br/><br/><input type=\"submit\" value=\"SUBMIT\"/></h4></form></body></html>\r\n";
	//httpdSend(connData,buff,strlen(buff));
	char *p;
	char *q;
	httpdSend(connData,html,strlen(html));
	if(connData->getArgs!=NULL)
	{
		p=strchr(connData->getArgs,'=');
		q=strchr(connData->getArgs,'&');
		*q='\0';
		on_time=atoi(++p);
		p=strchr(++q,'=');
		off_time=atoi(++p);
...
int cgiCounter(HttpdConnData *connData)
{
	char buff[30];
...
static void   httpdParseHeader(char *h, HttpdConnData *conn) {
	...
	//first_line表示请求的第一行（GET ... or POST ...）
    if (first_line) {
        char *e;
...
static void   httpdRecv(void *arg, char *data, unsigned short len) {
	...
	/*从收到的数据中找出head部分，放入HttpdConnData结构体*/
    for (x=0; x<len; x++) {
        if (conn->post->len<0) {
...
void   httpdInit(HttpdBuiltInUrl *fixedUrls, int port)
{
    ...
    printf("enter httpInit\r\n");
    /*
    wifi_softap_get_config(config); // Get soft-AP config first.
	sprintf(config->ssid, "Plug");
	sprintf(config->password, "hisense1");
	...
	*/
    ...
    sprintf(ap_config.ssid,"Plug");
	sprintf(ap_config.password,"hisense-sx");
	...
	if(wifi_softap_set_config(&ap_config)==TRUE)
		printf("ap set success\r\n");
	else
		printf("ap set failed\r\n");

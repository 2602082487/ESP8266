...
#define ERR_UNPACK_CMDREQ_REMAIN              -1031
...
/* sava_data json (C->S) */
EdpPacket *PacketSavedataJson(const char *dst_devid, char *json_str);
EdpPacket *PacketSaveJson(const char *dst_devid, char *json_str);
/* sava_data bin (C->S) */
EdpPacket *PacketSavedataBin(const char *dst_devid, char *json_str, uint8 *bin_data, uint32 bin_len);
/* ping (C->S) */
EdpPacket *PacketPing(void);
EdpPacket *PacketCmdresp(const char *cmdid, uint8 *cmdbody, uint32 bodylen);
EdpPacket *PacketEncryptresp(uint8 *secret_key, uint32 len);
EdpPacket *PacketSaveField(const char *dst_devid, const char *field);
...
int32 UnpackSavedataJson(EdpPacket *pkg, char **json_str);
int32 UnpackSavedataBin(EdpPacket *pkg, char **json_str, uint8 **bin_data, uint32 *bin_len);
/* ping_resp (S->C) */
